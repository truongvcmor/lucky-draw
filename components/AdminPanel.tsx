...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const triggerClearAll = () => {
    setConfirmState({
      isOpen: true,
      title: "CẢNH BÁO: XÓA TẤT CẢ",
      message: "Hành động này sẽ xóa sạch danh sách người chơi hiện tại. Bạn sẽ phải nhập lại từ đầu. Bạn có chắc không?",
      confirmLabel: "Xóa Sạch",
      confirmColor: "red",
      onConfirm: () => {
        onClearAllParticipants();
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex justify-end animate-fade-in" onClick={onClose}>
        <div 
          className="w-full max-w-2xl bg-[#0B1E33] h-full flex flex-col shadow-2xl border-l border-white/10 overflow-hidden" // Added overflow-hidden
          onClick={(e) => e.stopPropagation()} 
        >
          ...
          {/* TAB CONTENT CONTAINER (FLEX COLUMN) */}
          <div className="flex-1 min-h-0 flex flex-col relative bg-[#0B1E33]">
            
            {/* === PARTICIPANTS TAB === */}
            {activeTab === 'PARTICIPANTS' && (
              <div className="flex flex-col h-full">
                
                {/* 1. FIXED TOP SECTION (Stats + Form) */}
                <div className="flex-shrink-0 p-6 pb-0 space-y-6 bg-[#0B1E33] z-10">
                    {/* Stats */}
                    <div className="grid grid-cols-3 gap-4">
                        <div className="bg-gray-800/50 p-4 rounded-xl border border-white/5 text-center">
                        <div className="text-gray-400 text-xs uppercase font-bold mb-1">Tổng số</div>
                ...
                <div className="flex-1 min-h-0 p-6 pt-4 flex flex-col">
                    <div className="bg-gray-800/50 rounded-xl border border-white/5 flex flex-col h-full overflow-hidden">
                        {/* List Header (Sticky inside the box) */}
                        <div className="p-4 border-b border-gray-700 flex flex-wrap justify-between items-center bg-gray-800 shrink-0 gap-2">
                            <div className="flex items-center gap-4">
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center">
                                <span className="w-2 h-2 rounded-full bg-green-500 mr-2"></span>
                                Danh sách
                        ...
                        <div className="flex-1 overflow-y-auto relative scrollbar-thin scrollbar-thumb-gray-600">
                            <table className="w-full text-left border-collapse">
                                <thead className="text-xs text-gray-400 uppercase bg-gray-900/95 sticky top-0 z-10 shadow-md">
                                <tr>
                                    <th className="p-3 w-16 text-center font-bold">Số</th>
                                    <th className="p-3 font-bold">Thông tin</th>
                                    <th className="p-3 text-center w-24 font-bold">Trạng thái</th>
                                ...
                                {filteredParticipants.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="p-8 text-center text-gray-500 italic">Không tìm thấy dữ liệu phù hợp</td>
                                    </tr>
                                ) : (
                                    filteredParticipants.map(p => {
                                        const isBlacklisted = blacklistedNumbers.includes(p.assignedNumber);
            ...
            {activeTab === 'PRIZES' && (
              <div className="flex flex-col h-full">
                {/* Fixed Top */}
                <div className="flex-shrink-0 p-6 pb-0 bg-[#0B1E33] z-10">
                   {/* Prize Input */}
                    <div className={`bg-gray-800/50 rounded-xl border p-5 transition-colors ${editingPrizeId ? 'border-mor-orange bg-mor-orange/5' : 'border-white/5'}`}>
                        <h3 className="text-sm font-bold text-mor-gold uppercase tracking-widest mb-4 flex items-center justify-between">
                            <span className="flex items-center">
                ...
                <div className="flex-1 min-h-0 p-6 pt-6 flex flex-col">
                     <div className="bg-gray-800/50 rounded-xl border border-white/5 flex flex-col h-full overflow-hidden">
                        <div className="p-4 border-b border-gray-700 bg-gray-800 rounded-t-xl shrink-0">
                        <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center">
                            <span className="w-2 h-2 rounded-full bg-mor-gold mr-2"></span>
                            Danh sách giải thưởng
                        </h3>
